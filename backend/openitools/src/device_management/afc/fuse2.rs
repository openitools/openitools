...
impl AfcFS {
    pub fn new(files_path: Vec<String>, afc: AfcClient) -> Self {
...
impl Filesystem for AfcFS {
    ...
    async fn lookup(&self, _req: Request, parent: u64, name: &OsStr) -> FResult<ReplyEntry> {
        ...
        if let Some(&ino) = self.reverse_map.get(name_str) {
            let file = self
                .inode_map
                .get(&ino)
    ...
    async fn getattr(
        ...
        if let Some(file) = self.inode_map.get(&inode) {
            let File { path, .. } = &*file;
    ...
    async fn read(
        ...
        let file = match self.inode_map.get(&inode) {
            Some(p) => p,
        ...
        let mut remote_file = tokio::io::BufReader::new(match file {
            File { path, fd: fd } if fd.read().is_some() => unsafe {
                let fd = fd.read().unwrap_unchecked();
                println!("opening a file from fd: {fd}");
                FileDescriptor::new(&mut afc_lock, fd, path.clone())
            },
            File { path, fd } => {
                ...
                *fd.write() = Some(remote_file.as_raw_fd());
    ...
    async fn readdir<'a>(
        ...
        let mut entries = Vec::with_capacity(2 + self.reverse_map.len());
    ...
    ) -> FResult<ReplyDirectoryPlus<impl Stream<Item = FResult<DirectoryEntryPlus>> + Send + 'a>>
    {
        ...
        let mut entries: Vec<DirectoryEntryPlus> = Vec::with_capacity(2 + self.reverse_map.len());
