...
async fn run_installation(
    ...
    let provider = openitools_idevice::get_provider().await?;

    let mut lockdownd_client = openitools_idevice::get_lockdownd_client(&provider).await?;
    ...
    let connected_model = openitools_idevice::get_device_model(&mut lockdownd_client).await?;
    let connected_ios_version =
        openitools_idevice::get_device_ios_version(&mut lockdownd_client).await?;
